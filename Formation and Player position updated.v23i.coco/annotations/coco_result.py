...
def coco_to_per_image_json(coco_json_path, output_dir="output_json"):
    """
    Convert COCO annotation JSON into per-image JSON files with offense and defense player coordinates:
    {
        "image_path": "...",
        "x_def": [],
        "y_def": [],
    ...
    for ann in coco_data.get("annotations", []):
        ...
        cls_name = categories.get(category_id, "unknown").lower()
        ...
        anns_per_image[image_id]["cls"].append(cls_name)
        anns_per_image[image_id]["cls_off"].append(cls_name)
        anns_per_image[image_id]["cls_def"].append(cls_name)

        if x is not None and y is not None:
            if "offense" in cls_name:
                anns_per_image[image_id]["x_off"].append(x)
                anns_per_image[image_id]["y_off"].append(y)
            elif "defense" in cls_name:
    ...
    for img in coco_data.get("images", []):
        ...
        img_json = {
            ...
            "cls": ann_data["cls"],
            "cls_off": ann_data["cls_off"],
            "cls_def": ann_data["cls_def"]
        }
